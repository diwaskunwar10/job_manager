...
const JobComponent: React.FC<JobComponentProps> = ({ tenantName }) => {
  ...
  // Fetch jobs from API - This effect runs when job pagination changes
  useEffect(() => {
    const fetchJobs = async () => {
      setIsJobsLoading(true);
      try {
        const response = await JobService.getJobs(jobsCurrentPage, jobsPageSize);
  ...
  // Fetch agents from API - This effect runs only when agent pagination changes or on initial load
  useEffect(() => {
    ...
    fetchAgents();
  }, [agentsPageSize]); // Only depends on agentsPageSize
  ...
  const handleJobsPageChange = (page: number) => {
    ...
    // The useEffect hook will trigger a new API call with updated pagination params
  };
  ...
  const handleAgentsPageChange = (page: number) => {
    ...
    // Note: We're not fetching agents again here since we're using client-side pagination
    // If the API supports server-side pagination for agents, we would need to add
    // agentsCurrentPage to the dependency array of the agents useEffect
  };
  ...
  const handleAgentsPageSizeChange = (size: number) => {
    ...
    // This will trigger the agents useEffect because agentsPageSize is in its dependency array
  };
